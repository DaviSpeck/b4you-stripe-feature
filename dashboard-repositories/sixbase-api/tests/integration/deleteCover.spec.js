...
const makeSut = () => {
  ...
  const sut = new DeleteCover(productsRepoStub, fileManagerStub);
  return {
    sut,
    productsRepoStub,
    fileManagerStub,
...
describe('Delete cover use case', () => {
  ...
  it('should throw error if product doesnt have cover', async () => {
    const { sut, productsRepoStub } = makeSut();
    jest
  ...
  it('should call file manager with correct value', async () => {
    ...
    expect(fileManagerSpy).toHaveBeenCalledWith('valid_cover_key');
  });
});
