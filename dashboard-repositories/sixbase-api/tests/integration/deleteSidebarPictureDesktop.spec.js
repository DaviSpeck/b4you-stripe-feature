...
const DeleteSidebarPictureDesktop = require('../../useCases/dashboard/products/images/DeleteSidebarPictureDesktop');
...
const makeSut = () => {
  ...
  const sut = new DeleteSidebarPictureDesktop(
    productsRepoStub,
    fileManagerStub,
  );
  return {
...
describe('Delete sidebar picture desktop use case', () => {
  ...
  it('should throw error if product doesnt have sidebar picture desktop', async () => {
    const { sut, productsRepoStub } = makeSut();
    jest
    ...
    expect(error).toBeDefined();
    expect(error.code).toBe(400);
    expect(error.message).toBe('Produto não possui imagem sidebar desktop');
  });
  ...
  it('should call file manager with correct value', async () => {
    ...
    expect(fileManagerSpy).toHaveBeenCalledWith('valid_sidebar_key');
  });
});
