...
class ActiveCampaign {
  ...
  /**
   * @returns {Tags} Return all tags
   */
  async getAllTags() {
    const response = await this.#service.get('/tags', {
      headers: this.headers,
      params: { limit: 100 },
    });
    return response;
  ...
  /**
   * @returns {Tags} Return tags by ids
   */
  async getTagsByIds(tagsIds) {
    const tags = await Promise.all(
      tagsIds.map(async (id) => {
        const { data } = await this.#service.get(`/tags/${id}`, {
          headers: this.headers,
        });
        return data;
      }),
