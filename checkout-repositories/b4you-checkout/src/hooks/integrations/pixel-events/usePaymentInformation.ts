...
export function usePaymentInformationEvents(offerData: iOffer) {
  ...
  function handler(payload: ContractDataType) {
    ...
    const addPaymentInfoCookie = Cookies.get("add-payment-info");

    if (
      addPaymentInfoCookie ||
    ...
    Cookies.set("add-payment-info", "true", { expires: expirationDate });
