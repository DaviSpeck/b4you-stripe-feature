...
const ModalPlugin = ({ uuidIntegration, setShow }) => {
  ...
  const fetchData = () => {
    api
      .get('/integrations/events/astronmembers')
      .then((response) => {
  ...
  const onSubmit = (data) => {
    ...
    api
      .post(
        `/integrations/astronmembers/product/${uuidProduct}/plugin/${uuidIntegration}`,
        fields
      )
      .then(() => {
        notify({
  ...
  return (
    <>
      {events !== null && products !== null ? (
        ...
        <>
          <Loader title='Carregando dados de astronmembers...' />
        </>
      )}
    </>
  );
