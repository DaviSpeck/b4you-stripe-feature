const fs = require('fs');
const UpdateHeaderPictureDesktop = require('../../useCases/dashboard/products/images/UpdateHeaderPictureDesktop');
...
const makeSut = () => {
  ...
  const sut = new UpdateHeaderPictureDesktop(productsRepoStub);
  return {
    sut,
    productsRepoStub,
  };
...
describe('Update product header picture desktop', () => {
  ...
  it('should throw if image helper throws', async () => {
    ...
    ImageHelper.resizeImageHeaderDesktop.mockReturnValueOnce(
      new Promise((resolve, reject) => {
        reject(new Error());
      }),
    );
