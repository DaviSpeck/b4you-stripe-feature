...
const DeleteBanner = require('../../useCases/dashboard/products/images/DeleteBanner');
...
const makeSut = () => {
  ...
  const sut = new DeleteBanner(productsRepoStub, fileManagerStub);
  return {
    sut,
    productsRepoStub,
    fileManagerStub,
...
describe('Delete favicon use case', () => {
  ...
  it('should throw error if product doesnt have banner', async () => {
    const { sut, productsRepoStub } = makeSut();
    jest
    ...
    expect(error).toBeDefined();
    expect(error.code).toBe(400);
    expect(error.message).toBe('Produto não possui uma imagem banner');
  });
  ...
  it('should call file manager with correct value', async () => {
    ...
    expect(fileManagerSpy).toHaveBeenCalledWith('valid_banner_key');
  });
});
