const fs = require('fs');
const UpdateHeaderPictureMobile = require('../../useCases/dashboard/products/images/UpdateHeaderPictureMobile');
...
const makeSut = () => {
  ...
  const sut = new UpdateHeaderPictureMobile(productsRepoStub);
  return {
    sut,
    productsRepoStub,
  };
...
describe('Update product header picture desktop', () => {
  ...
  it('should throw if image helper throws', async () => {
    ...
    ImageHelper.resizeHeaderMobile.mockReturnValueOnce(
      new Promise((resolve, reject) => {
        reject(new Error());
      }),
    );
