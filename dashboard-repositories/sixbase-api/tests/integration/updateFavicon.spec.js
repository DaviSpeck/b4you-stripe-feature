const fs = require('fs');
const UpdateFavicon = require('../../useCases/dashboard/products/images/UpdateFavicon');
...
const makeSut = () => {
  ...
  const sut = new UpdateFavicon(productsRepoStub);
  return {
    sut,
    productsRepoStub,
  };
...
describe('Update product header picture desktop', () => {
  ...
  it('should throw if image helper throws', async () => {
    ...
    ImageHelper.resizeFavicon.mockReturnValueOnce(
      new Promise((resolve, reject) => {
        reject(new Error());
      }),
    );
