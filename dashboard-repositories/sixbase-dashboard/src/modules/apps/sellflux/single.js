...
const PageAppsListSellfluxSingle = () => {
  ...
  const fetchData = () => {
    api
      .get(`/integrations/sellflux/${uuidIntegration}`)
      .then((response) => {
  ...
  const handleRemove = (item) => {
    api
      .delete(`/integrations/sellflux/${uuidIntegration}/${item.uuid}`)
      .then(() => {
        notify({
  ...
  return (
    <>
      ...
      <PageTitle
        ...
        path={[
          { url: '/apps', text: 'Apps' },
          { url: '/apps/sellflux', text: `Sellflux` },
        ]}
      />
...
export default PageAppsListSellfluxSingle;
