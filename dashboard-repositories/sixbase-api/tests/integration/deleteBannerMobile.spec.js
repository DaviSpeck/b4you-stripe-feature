...
const DeleteBannerMobile = require('../../useCases/dashboard/products/images/DeleteBannerMobile');
  ...
  class ProductsRepository {
    static async findWithProducer() {
      return new Promise((resolve) => {
        resolve({
          id: 'valid_id',
          banner_mobile_key: 'valid_banner_mobile_key',
        });
      });
    }
...
const makeSut = () => {
  ...
  const sut = new DeleteBannerMobile(productsRepoStub, fileManagerStub);
  return {
    sut,
    productsRepoStub,
    fileManagerStub,
...
describe('Delete banner mobile use case', () => {
  ...
  it('should throw error if product doesnt have banner mobile', async () => {
    const { sut, productsRepoStub } = makeSut();
    jest
    ...
    expect(error).toBeDefined();
    expect(error.code).toBe(400);
    expect(error.message).toBe('Produto não possui uma imagem banner mobile');
  });
  ...
  it('should call file manager with correct value', async () => {
    ...
    expect(fileManagerSpy).toHaveBeenCalledWith('valid_banner_mobile_key');
  });
});
