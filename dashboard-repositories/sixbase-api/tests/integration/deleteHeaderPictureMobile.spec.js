...
const DeleteHeaderPictureMobile = require('../../useCases/dashboard/products/images/DeleteHeaderPictureMobile');
  ...
  class ProductsRepository {
    static async findWithProducer() {
      return new Promise((resolve) => {
        resolve({
          id: 'valid_id',
          header_picture_mobile_key: 'valid_header_key',
        });
      });
    }
...
const makeSut = () => {
  ...
  const sut = new DeleteHeaderPictureMobile(productsRepoStub, fileManagerStub);
  return {
    sut,
    productsRepoStub,
    fileManagerStub,
...
describe('Delete header picture mobile use case', () => {
  ...
  it('should throw error if product doesnt have header picture mobile', async () => {
    const { sut, productsRepoStub } = makeSut();
    jest
    ...
    expect(error).toBeDefined();
    expect(error.code).toBe(400);
    expect(error.message).toBe('Produto não possui uma header mobile');
  });
