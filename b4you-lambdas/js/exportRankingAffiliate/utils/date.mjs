...
const dateHelper =
  ...
  (date = new Date(), dateFormat = "") => {
    ...
    return {
      ...
      formatFromUnix: (unixTime, outputFormat) =>
        dateParser.unix(unixTime).format(outputFormat),
      ...
      utcOffset: (offset, keepTimeOfDay = false) =>
        Date.utcOffset(offset, keepTimeOfDay),
      utc: (keepTimeOfDay = false) => Date.utc(keepTimeOfDay),
    };
  };

export const date = dateHelper(moment);
export const DATABASE_DATE = "YYYY-MM-DD HH:mm:ss";
