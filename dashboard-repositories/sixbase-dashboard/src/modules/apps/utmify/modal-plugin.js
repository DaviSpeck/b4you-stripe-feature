...
const ModalPlugin = ({ uuidIntegration, setShow }) => {
  ...
  const fetchData = () => {
    api
      ...
      .then((response) => {
        setEvents(response.data.filter((c) => eventsIds.includes(c.id)));
      })
      .catch(() => {})
      .finally();
  ...
  const onSubmit = (data) => {
    ...
    api
      .post(
        `/integrations/utmify/product/${uuidProduct}/plugin/${uuidIntegration}`,
        fields
      )
      .then(() => {
        notify({
  ...
  return (
    <>
      {events !== null && products !== null ? (
        ...
        <>
          <Loader title='Carregando dados de Cademi...' />
        </>
      )}
    </>
  );
