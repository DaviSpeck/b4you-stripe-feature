...
- **Jornada – Domínios**: tabela paginada com volume de sessões e conversão por domínio.
...
- **Pagamento** → seleção de método, sucesso e erros de pagamento.
...
- **Conversão por método de pagamento**: comparação de volume e taxa de sucesso.
...
- **Sessões com conversão concluída**: sessões com `checkout_conversion_success`.
- **Sessões com pagamento aprovado**: sessões com `checkout_payment_success` (fallback).
...
- **Domínios**: tabela com paginação por página (root_domain).
...
## Tracking — classificação de checkoutMode
Para manter consistência nos eventos enviados pelo frontend, `checkoutMode`
agora aceita os valores:
...
- `localhost` ou IP (ex.: `127.0.0.1`, `192.x.x.x`) ⇒ `development`
