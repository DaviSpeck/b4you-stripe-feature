  ...
  class ProductsRepository {
    static async findWithProducer() {
      return new Promise((resolve) => {
        resolve({
          id: 'valid_id',
          second_header_mobile_key: 'valid_second_header',
        });
      });
    }
...
const makeSut = () => {
  ...
  const sut = new DeleteSecondHeaderMobile(productsRepoStub, fileManagerStub);
  return {
    sut,
    productsRepoStub,
    fileManagerStub,
...
describe('Delete second header mobile use case', () => {
  ...
  it('should throw error if product doesnt have second header mobile', async () => {
    const { sut, productsRepoStub } = makeSut();
    jest
  ...
  it('should call file manager with correct value', async () => {
    ...
    expect(fileManagerSpy).toHaveBeenCalledWith('valid_second_header');
  });
});
