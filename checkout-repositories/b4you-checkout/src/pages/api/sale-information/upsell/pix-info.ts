...
export default apiHandler(async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const bodyData = "encrypted" in req.body ? decryptData(req.body) : req.body;

  try {
    const response = await apiExternal.post("/sales/process-upsell", {
      ...bodyData,
      payment_method: "pix",
      installments: 1,
    });
    ...
    return res.status(500).json({} as iSaleData);
  }
});