...
const SerializePlugins = require('../../../presentation/dashboard/bling-shipping/get');
const BlingShipping = require('../../../services/integrations/BlingShipping');
...
module.exports.createIntegrationController = async (req, res, next) => {
  ...
  try {
    const plugins = await findPlugins({
      id_user,
      id_plugin: findIntegrationTypeByKey('blingshipping').id,
    });
    ...
    const integration = new BlingShipping(api_key);
...
module.exports.getIntegrationsController = async (req, res, next) => {
  ...
  try {
    ...
    const integration = new BlingShipping(plugins[0].settings.api_key);
...
module.exports.updateIntegrationController = async (req, res, next) => {
  ...
  try {
    ...
    return res.status(200).send({
      success: true,
      message: 'Integração com BlingShipping atualizada com sucesso',
...
module.exports.deleteIntegrationController = async (req, res, next) => {
  ...
  try {
    ...
    return res.status(200).send({
      success: true,
      message: 'Integração com BlingShipping excluída com sucesso',
    });
  } catch (error) {
    if (error instanceof ApiError) return res.status(error.code).send(error);
    return next(
