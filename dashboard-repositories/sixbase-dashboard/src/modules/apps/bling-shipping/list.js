...
import logoBling from '../../../images/apps/bling-shipping-legacy.png';
...
const PageAppsListBling = () => {
  ...
  const handleDelete = () => {
    setRequesting(true);
    api
      .delete(`/integrations/bling-shipping/`)
      .then(() => {
        fetchData();
  ...
  const fetchData = () => {
    setRequesting(true);
    api
      .get('/integrations/bling-shipping')
      .then((response) => {
  ...
  return (
    <>
      <PageTitle
        title='Bling Pedidos de Venda'
        path={[
          { url: '/apps', text: 'Apps' },
          { url: null, text: 'Bling Pedidos de Venda' },
        ]}
      />
      <ModalGeneric
        show={showModal}
      ...
      <section id='page-apps'>
        ...
        <Card className='mt-4'>
         ...
         <b>Atenção:</b>  A versão atual da integração com o Bling não é mais suportada. Recomendamos que você migre para a Versão 3 para continuar utilizando todos os recursos disponíveis e garantir compatibilidade com futuras atualizações.
        </Alert>
        <ButtonDS onClick={() => navigate('/apps/bling-shipping-v3')}>
          Migrar para a Versão 3
        </ButtonDS>
