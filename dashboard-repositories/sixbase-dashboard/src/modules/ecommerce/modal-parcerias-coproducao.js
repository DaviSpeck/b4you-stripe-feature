...
const CoproductionContent = ({ productUuid, product }) => {
  ...
  const fetchData = () => {
    ...
    api
      .get('/products/coproduction/' + productUuid)
      .then((response) => {
        setTimeout(() => {
          setRequesting(false);
  ...
  const onSubmit = (data) => {
    ...
    api
      .post('/products/coproduction/' + productUuid + '/invite', data)
      .then(() => {
  ...
  const handleChangeCommission = () => {
    ...
    api
      .put(
        `/products/coproduction/${productUuid}/${activeCoproduction.uuid}`,
        fields
      )
      .then(() => {
        ...
        <ConfirmAction
         ...
         coprodutor novamente.
          Você tem certeza que quer cancelar o contrato de ${
            activeCoproduction?.commission_percentage || 0
          }% com o usuário
...
export default CoproductionContent;
