...
const PageProductsEditManager = () => {
  ...
  const fetchData = () => {
    api.get('/products/managers/' + uuidProduct).then((response) => {
  ...
  const fetchCountAffiliates = (manager) => {
    api
      .get('/products/managers/' + uuidProduct + '/count/' + manager.id)
  ...
  const handleCancel = () => {
    api
      .delete('/products/managers/' + uuidProduct + `/` + activeManager.id)
      .then(() => {
        notify({
...
export default PageProductsEditManager;
