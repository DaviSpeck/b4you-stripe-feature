export function FormatterCnpj(value) {
  const rawCnpj = value.replace(/[^0-9]/g, '');
  ...
  if (rawCnpj.length <= 12)
    return `${rawCnpj.slice(0, 2)}.${rawCnpj.slice(2, 5)}.${rawCnpj.slice(
      5,
      8
    )}/${rawCnpj.slice(8)}`;
  ...
  return `${rawCnpj.slice(0, 2)}.${rawCnpj.slice(2, 5)}.${rawCnpj.slice(
    5,
    8
  )}/${rawCnpj.slice(8, 12)}-${rawCnpj.slice(12, 14)}`;
}
