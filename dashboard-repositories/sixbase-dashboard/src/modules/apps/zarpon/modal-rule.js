...
const ModalRule = ({ fetchData, setShow, webhook }) => {
  ...
  const fetchProducts = () => {
    api
      ...
      .then((response) => {
        const { rows, affiliates } = response.data;
        setProducts(rows);
  ...
  const renderOptions = (affiliatesProducts, products) => {
    const options = [
  ...
  return (
    <section id='modal-rule-webhooks'>
      <Row>
        ...
        <Col xs={12}>
          <div className='form-group'>
            <label htmlFor=''>URL da Integração</label>
            <InputGroup>
              <Form.Control
                ref={register({
                  required: true,
        ...
        <Col xs={12}>
          <div className='form-group'>
            ...
            >
              {renderOptions(affiliateProducts, products)}
            </Form.Control>
          </div>
        </Col>
        <Col xs={12}>
