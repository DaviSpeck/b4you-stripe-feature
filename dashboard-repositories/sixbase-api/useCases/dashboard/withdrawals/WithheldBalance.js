module.exports = class WithheldBalance {
  ...
  async calculate(id_user, withheld_balance_percentage, use_highest_sale) {
    const lastThirdDaysCommissions =
      await this.#CommissionsRepository.sum30DaysTotal(id_user);
    ...
    const highestSale = await this.#CommissionsRepository.findHighestSale(
      id_user,
    );
