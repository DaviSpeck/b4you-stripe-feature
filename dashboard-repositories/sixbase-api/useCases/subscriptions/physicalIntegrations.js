...
const Sales_invoices = require('../../database/models/Sales_invoices');
const Sales_items = require('../../database/models/Sales_items');
const Sales_items_plugins = require('../../database/models/Sales_items_plugins');
...
const productPhysicalEvents = async ({
  ...
  // gravando id do pedido na tabela da shopify
  if (shopifyResponse?.id) {
    await Sales_items_plugins.create({
      id_sale_item,
      id_shopify: shopifyResponse.id,
      body: shopifyData,
      response:
        Object.keys(shopifyResponse).length > 0 ? shopifyResponse : null,
    });
  }
  ...
  //     );
  //     const responseShopify = await sendToShopify({
  //       id_offer,
  //       id_sale,
  //       plugin: pluginShopify,
  //     });
  //     console.log(
  //       `[SHOPIFY] Pedido enviado com sucesso para Shopify`,
  //       responseShopify,
  //     );
  //   } else {
  //     console.log(
