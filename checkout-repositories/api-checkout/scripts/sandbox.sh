...
API_PORT_DEFAULT=$((5500 + (HASH % 1000)))
API_PORT="${PORT_OVERRIDE:-$API_PORT_DEFAULT}"
REDIS_PORT=6380
...
case "$cmd" in
  up|rebuild)
    ...
    docker build -f "$DOCKERFILE" -t "sixbase-api-checkout:$BRANCH_NAME" "$REPO_ROOT"
  ...
  rm)
    ...
    docker rmi -f "sixbase-api--checkout:$BRANCH_NAME" || true
    ;;
