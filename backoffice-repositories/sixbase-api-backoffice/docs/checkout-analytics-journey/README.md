...
```json
{
  ...
  "checkout_mode": "embedded|transparent|sandbox|development?",
...
- `environment`, `has_success` e `has_error` são opcionais e afetam todas as consultas.
...
```json
{
  ...
  "conversion_success_sessions": 280,
  "payment_success_sessions": 260,
  "error_sessions": 190
}
```

`success_sessions` representa o total de sessões com `checkout_conversion_success`
ou `checkout_payment_success`. Os campos separados permitem visualizar a adoção do
novo evento de conversão.
...
```json
{
  "steps": [
    ...
    { "event_name": "checkout_conversion_success", "label": "Conversão concluída", "sessions": 310 }
  ]
}
```
...
### Domínios (paginado)
`POST /checkout/analytics/journey/domains`

```json
{
  "items": [
    {
      "root_domain": "exemplo.com.br",
      "sessions": 210,
      "success_sessions": 90
    }
  ],
...
Na interface, `root_domain` pode ser exibido como “Domínio não identificado”
