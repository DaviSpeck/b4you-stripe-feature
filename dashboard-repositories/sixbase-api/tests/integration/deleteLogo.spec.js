...
const makeSut = () => {
  ...
  const sut = new DeleteLogo(productsRepoStub, fileManagerStub);
  return {
    sut,
    productsRepoStub,
    fileManagerStub,
...
describe('Delete logo use case', () => {
  ...
  it('should throw error if product doesnt have logo', async () => {
    const { sut, productsRepoStub } = makeSut();
    jest
    ...
    expect(error).toBeDefined();
    expect(error.code).toBe(400);
    expect(error.message).toBe('Produto sem logo');
  });
  ...
  it('should call file manager with correct value', async () => {
    ...
    expect(fileManagerSpy).toHaveBeenCalledWith('valid_logo_key');
  });
});
