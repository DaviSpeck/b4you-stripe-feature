...
import { customStyles } from './tag-select.style';
...
const ModalRule = ({ uuidIntegration, setShow }) => {
  ...
  const onSubmit = (data) => {
    ...
    let fields = {
      ...
      ids_tags: tagsSelected.map((item) => item.value),
    };

    api
      .post(
        `/integrations/activecampaign/product/${uuidProduct}/plugin/${uuidIntegration}`,
        fields
      )
      .then(() => {
        setShow(false);
  ...
  const fetchData = () => {
    ...
    api
      .get(`/integrations/activecampaign/lists/${uuidIntegration}`)
      .then((response) => {
    ...
    api
      .get(`/integrations/activecampaign/tags/${uuidIntegration}`)
      .then((response) => {
        const dataFormatted = response.data.map((elem) => ({
          label: elem.tag,
          value: elem.id,
        }));

        setTags(dataFormatted);
      })
      .catch(() => {})
      .finally();
  };
