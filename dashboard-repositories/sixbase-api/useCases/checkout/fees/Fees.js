...
module.exports = class Fees {
  ...
  sale() {
    ...
    let [sale_base_price, subscription_fee_sum, discount_amount] =
      this.sales_items.reduce(
        (
          acc,
          { price: sale_price, subscription_fee = 0, shipping_price = 0, type },
        ) => {
          ...
          acc[2] +=
            totalPrice *
            (type === 1
              ? discount_decimal + discount_coupon_decimal
              : discount_decimal);
          return acc;
        },
        [0, 0, 0],
    ...
    if (this.student_pays_interest) {
      ...
      if (this.installments === 1) {
        ...
        interest_installment_amount = Number(
          Number((pmt * this.installments - sale_base_price).toFixed(2)),
        ); // juros de parcelamento
        interest_installment_percentage =
          (interest_installment_amount / sale_base_price) * 100; // percentual dos juros de parcelamento
    ...
    for (const { price, type, subscription_fee = 0, shipping_price = 0 } of this
      ...
      if (this.student_pays_interest) {
        ...
        if (this.installments === 1) {
          ...
          interest_installment_percentage =
            (interest_installment_amount / price_base) * 100;
      ...
      transactions.push({
        ...
        discount_amount: discount_amount_sale_item,
        discount_percentage:
          discount_percentage + (type === 1 ? this.coupon_discount : 0),
        original_price: original_price_sale_item,
