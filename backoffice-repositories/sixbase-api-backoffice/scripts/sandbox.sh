...
PROJECT="backoffice-${BRANCH_NAME}"

case "$cmd" in
  up|rebuild)
    # Build imagem única por branch
    docker build -f "$DOCKERFILE" -t "sixbase-api-backoffice:$BRANCH_NAME" "$REPO_ROOT"
  ...
  rm)
    ...
    docker rmi -f "sixbase-api-backoffice:$BRANCH_NAME" || true
    ;;
