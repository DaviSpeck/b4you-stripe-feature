...
export default class ChargebackWin {
  ...
  async execute() {
      ...
      for await (const comm of commissions) {
        this.t.afterCommit(async () => {
          await aws.add('usersRevenue', {
            ...
            amount: comm.amount,
            operation: 'increment',
            paid_at: date(saleItem.paid_at).subtract(3, 'hours').format(DATABASE_DATE_WITHOUT_TIME),
          });
    ...
    console.log(`CHARGEBACK WIN CONFIRMED WITH PSP ID ${this.psp_id}`);
    return true;
  }
}
