const fs = require('fs');
const UpdateSidebarPictureDesktop = require('../../useCases/dashboard/products/images/UpdateSidebarPictureDesktop');
...
const makeSut = () => {
  ...
  const sut = new UpdateSidebarPictureDesktop(productsRepoStub);
  return {
    sut,
    productsRepoStub,
  };
...
describe('Update product header picture desktop', () => {
  ...
  it('should throw if image helper throws', async () => {
    ...
    ImageHelper.resizeImageSidebar.mockReturnValueOnce(
      new Promise((resolve, reject) => {
        reject(new Error());
      }),
    );
