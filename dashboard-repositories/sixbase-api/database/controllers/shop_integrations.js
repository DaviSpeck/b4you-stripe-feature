...
/**
 * Load shop integration with all data needed for checkout resolution
 * Includes: container_product and default_offer with order_bumps
 */
const findShopIntegrationForCheckout = async (shop_domain, shop_uuid = null) => {
  const where = shop_uuid ? { uuid: shop_uuid, active: true } : { shop_domain, active: true };
...
/**
 * Load shop integration with product and offer data
 */
...
const createShopIntegration = async (data) =>
  Shop_integrations.create(data);
...
const createOrderBump = async (data) =>
  Order_bumps.create(data);

const updateOrderBump = async (id, data) =>
  Order_bumps.update(data, { where: { id } });

const deleteOrderBump = async (id) =>
  Order_bumps.destroy({ where: { id } });
...
/**
 * Get all checkout data for a shop in a single optimized query
 * Uses the shop_checkout_data VIEW for better performance
 * @param {string} identifier - shop_domain or shop_uuid
 * @param {string} type - 'domain' or 'uuid' (default: 'domain')
 * @returns {Object} Complete checkout data with order_bumps
 */
const getCheckoutData = async (identifier, type = 'domain') => {
  ...
  // Parse JSON fields if they come as strings (depends on MySQL version)
  ...
  return {
    ...
    // Default offer configuration fields (for inheritance)
    ...
    offer_checkout_customizations: parseJson(results.offer_checkout_customizations),
...
/**
 * Get checkout data by shop domain
...
/**
 * Get checkout data by shop UUID
