...
const DeleteFavicon = require('../../useCases/dashboard/products/images/DeleteFavicon');
...
const makeSut = () => {
  ...
  const sut = new DeleteFavicon(productsRepoStub, fileManagerStub);
  return {
    sut,
    productsRepoStub,
    fileManagerStub,
...
describe('Delete favicon use case', () => {
  ...
  it('should throw error if product doesnt have favicon', async () => {
    const { sut, productsRepoStub } = makeSut();
    jest
    ...
    expect(error).toBeDefined();
    expect(error.code).toBe(400);
    expect(error.message).toBe('Produto não possui um favicon');
  });
  ...
  it('should call file manager with correct value', async () => {
    ...
    expect(fileManagerSpy).toHaveBeenCalledWith('valid_favicon_key');
  });
});
