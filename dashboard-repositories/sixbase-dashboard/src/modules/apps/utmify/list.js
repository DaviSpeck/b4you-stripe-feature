...
const PageAppsListUtmify = () => {
  ...
  const handleEdit = (item) => {
    history.push(`/apps/utmify/${item.uuid}`);
  };

  const fetchData = () => {
    api
      .get('/integrations/utmify')
      .then((response) => {
  ...
  const handleDelete = () => {
    api
      .delete(`/integrations/utmify/${itemCancel.uuid}`)
      .then(() => {
        fetchData();
...
export default PageAppsListUtmify;
