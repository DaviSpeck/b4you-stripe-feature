...
module.exports = class CoproductionsInvitesRepositoryMemory {
  ...
  static async find(where) {
    return new Promise((resolve) => {
      const { id_product, id_coproducer, id_productor, status, id } = where;
      if (Object.keys(where).length === 0) return resolve(invites[0]);
      let result = invites;
      ...
      if (id_coproducer) {
        result = result.filter((c) => c.id_coproducer === id_coproducer);
        if (result.length === 0) return resolve(null);
      }
      if (id_product) {
      ...
      if (id_productor) {
        result = result.filter((r) => r.id_productor === id_productor);
        if (result.length === 0) return resolve(null);
      }
  ...
  static async findAll(where) {
    return new Promise((resolve) => {
      const { id_product, id_coproducer, id_productor, status } = where;
      if (Object.keys(where).length === 0) return resolve(invites);
      let result = invites;
      if (id_coproducer) {
        result = result.filter((c) => c.id_coproducer === id_coproducer);
  ...
  static async update(id, data) {
    return new Promise((resolve) => {
      ...
      const index = invites.findIndex((c) => c.id === invite.id);
      invites[index] = {
        ...invite,
        ...data,
      };
